...
class LlamaDecoderDAPLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # self.lang_prompt = Prompt(length=20, top_k=5, pool_size=10, embed_dim=4096, key_dim=4096)
        
        self.lang_prompt_downsample = nn.Linear(576, 100)
        nn.init.zeros_(self.lang_prompt_downsample.weight)
        nn.init.zeros_(self.lang_prompt_downsample.bias)
        self.lang_prompt_film = nn.Linear(1024, config.hidden_size * 2)
