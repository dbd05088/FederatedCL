...
class LlamaDecoderIA3PoolLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.lang_prompt_ia3_pool_1 = Pool(1,4096+4096+11008,1024,pool_size=config.pool_size,top_k=config.prompt_top_k)
        self.lang_prompt_ia3_pool_2 = Pool(1,4096+4096+11008,1024,pool_size=config.pool_size,top_k=config.prompt_top_k)
        ...
        **kwargs,
        ...
        if query_embeds is not None:
            if self.active_state == 'lora1':
                new_query_embeds, reduce_sim, _ = self.lang_prompt_ia3_pool_1(query_embeds, task_id)
            elif self.active_state == 'lora2':
                new_query_embeds, reduce_sim, _ = self.lang_prompt_ia3_pool_2(query_embeds, task_id)
            
            elif self.active_state == 'gate':
                new_query_embeds_1, _, _ = self.lang_prompt_ia3_pool_1(query_embeds, task_id)
                new_query_embeds_2, reduce_sim, _ = self.lang_prompt_ia3_pool_2(query_embeds, task_id)
...
class LlamaDAPForCausalLM(LlamaForCausalLM):
    def forward(
        ...
        outputs, l2p_key_losses = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
...
class LlavaLlamaForL2PIA3DualCausalLM(LlamaDAPForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig
