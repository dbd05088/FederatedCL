...
from models.dap_ia3.dapia3_attn import LlamaDAPIA3Attention
from models.dap_ia3.dapia3_mlp import LlamaDAPIA3MLP
...
class LlamaDecoderDAPIA3Layer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__(config, layer_idx)
        
        self.hidden_size = config.hidden_size

        self.self_attn = LlamaDAPIA3Attention(config=config, layer_idx=layer_idx)
        self.mlp = LlamaDAPIA3MLP(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states, idx=task_id_estimated_emb,query_embeds=query_embeds)
        hidden_states = residual + hidden_states
...
class LlamaDAPIA3Model(LlamaModel):
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderDAPIA3Layer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LlavaLlamaDAPIA3Model(LlavaMetaModel, LlamaDAPIA3Model):
    config_class = LlavaConfig

    def __init__(self, config: LlamaConfig):
        super(LlavaLlamaDAPIA3Model, self).__init__(config)


class LlamaDAPIA3ForCausalLM(LlamaForCausalLM):
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
...
class LlavaLlamaDAPIA3ForCausalLM(LlamaDAPIA3ForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig

    def __init__(self, config, prompt_num):
        super(LlamaForCausalLM, self).__init__(config)
        config.prompt_num = prompt_num
        self.model = LlavaLlamaDAPIA3Model(config)
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None,
                                      inputs_embeds=None, task_id_estimated_emb=None, query_embeds=None,cache_position=None, **kwargs):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, (task_id_estimated_emb,input_features,reduce_sim)