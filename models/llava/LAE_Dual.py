...
import copy
from models.lae_ia3.lae_ia3_layer import LAEIA3Layer
from models.lae_ia3_dual.dual_lae_ia3_layer import DualLAEIA3Layer
...
class LlavaLlamaForDualLAEIA3CausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig
    ...
    def set_lae_state(self, state):
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, LAEIA3Layer) or isinstance(module, DualLAEIA3Layer):
                module.set_lae_state(state)
    ...
    def set_state(self, state):
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, DualLAEIA3Layer):
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        past_key_values_offline = None
        past_key_values_online = None
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):

            # offline (ema updated module)
            model_kwargs1 = copy.deepcopy(model_kwargs)
            model_kwargs1['past_key_values'] = past_key_values_offline
            self.set_lae_state('lora1')
            ...
            past_key_values_offline = outputs.past_key_values

            # online (stream updated module)
            model_kwargs2 = copy.deepcopy(model_kwargs)
            model_kwargs2['past_key_values'] = past_key_values_online
            self.set_lae_state('lora2')
