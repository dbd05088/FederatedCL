...
class LlamaDecoderL2PLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # add prompt
        image_feature_indices = getattr(kwargs, 'image_feature_indices', None)
        bsz = hidden_states.shape[0]
        if image_feature_indices is not None:
            image_features = []
            for i in range(bsz):
                breakpoint()
                image_features.append(hidden_states[i,image_feature_indices, :])
...
class LlavaLlamaL2PForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaConfig
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, image_feature_indices