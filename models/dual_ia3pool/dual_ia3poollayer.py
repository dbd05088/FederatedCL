...
class DualIA3PoolLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, is_feedforward: bool, **kwargs) -> None:
        ...
        self.ia3_l_1 = nn.ParameterDict({})
        self.ia3_l_2 = nn.ParameterDict({})
    ...
    def update_layer(self, adapter_name, init_ia3_weights):
        ...
        if self.is_feedforward:
            weight = torch.ones((self.pool_size, 1, self.in_features))
        else:
            weight = torch.ones((self.pool_size, self.out_features, 1))
        self.ia3_l_1[adapter_name] = nn.Parameter(weight)
        self.ia3_l_2[adapter_name] = copy.deepcopy(self.ia3_l_1[adapter_name])
        if init_ia3_weights:
    ...
    def init_next_ia3(self, task_id):
        if task_id >= 0 and (task_id+1)*self.top_k < self.pool_size:
            for active_adapter in self.active_adapters:
                if active_adapter not in self.ia3_l_2.keys():
                    continue
                ...
                self.ia3_l_2[active_adapter][(task_id+1)*self.top_k:(task_id+2)*self.top_k,:,:].data = self.ia3_l_2[active_adapter][task_id*self.top_k:(task_id+1)*self.top_k,:,:]
            ...
            for active_adapter in self.active_adapters:
                ...
                if idx is not None:
                    ...
                    if self.active_state == 'lora1':
                        ...
                        ia3s = torch.stack([self.ia3_l_1[active_adapter][global_idx[i]] for i in range(bs)], dim=1)
                        ia3_scaling *= mean_ia3.reshape(bs, -1)
                    elif self.active_state =='lora2':
                        # choose by top_k
                        # ia3s = self.ia3_l_2[active_adapter][local_idx[0]]
                        ia3s = torch.stack([self.ia3_l_2[active_adapter][local_idx[i]] for i in range(bs)], dim=1)
                        # mean_ia3 = ia3s.mean(dim=0) if len(ia3s.shape) > 2 else ia3s
                        mean_ia3 = ia3s.mean(dim=0)
                        ia3_scaling *= mean_ia3.reshape(bs, -1)
                    elif self.active_state == 'gate':
                        # ia3s_1 = self.ia3_l_1[active_adapter][global_idx[0]]
                        ia3s_1 = torch.stack([self.ia3_l_1[active_adapter][global_idx[i]] for i in range(bs)], dim=1)
                        # mean_ia3_1 = ia3s_1.mean(dim=0) if len(ia3s_1.shape) > 2 else ia3s_1
                        # mean_ia3_1, _ = create_mask_gumbel(ia3s_1[0], ia3s_1[1])
                        # mean_ia3_1 = ia3s_1.mean(dim=0)
                        
                        # ia3s_2 = self.ia3_l_2[active_adapter][local_idx[0]]
                        ia3s_2 = torch.stack([self.ia3_l_2[active_adapter][local_idx[i]] for i in range(bs)], dim=1)
                        # mean_ia3_2 = ia3s_2.mean(dim=0) if len(ia3s_2.shape) > 2 else ia3s_2
                        # mean_ia3_2, _ = create_mask_gumbel(ia3s_2[0], ia3s_2[1])
                        ...
                        mean_ia3 = torch.cat((ia3s_1, ia3s_2),dim=0).mean(dim=0)
...
def create_mask_gumbel(tensor1, tensor2, tau=1.0, hard=False, is_training=False):
    ...
    one_greater_one_smaller = (tensor1 > 1) & (tensor2 < 1) | (tensor1 < 1) & (tensor2 > 1)
