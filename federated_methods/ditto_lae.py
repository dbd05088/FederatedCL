...
def ditto_lae_set_state_dict(model, global_state_dict, local_state_dict_list, training_args):
...
def ditto_lae_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict):
    trainer = LLaVATrainerDITTOLAE(model=model,
        tokenizer=tokenizer,
        args=training_args,
        packing=True,
...
class LLaVATrainerDITTOLAE(LLaVATrainerFEDAVG):   
    def __init__(self, global_state, ema_ratio, **kwargs):
        super(LLaVATrainerDITTOLAE, self).__init__(**kwargs)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], update_adapter) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs, update_adapter=update_adapter)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, update_adapter='lora1'):
        ...
        if update_adapter == 'lora1':
            ...
            loss_global, outputs = super(LLaVATrainerDITTOLAE, self).compute_loss(model, inputs, return_outputs=True)    
            ...
            loss_local, local_outputs = super(LLaVATrainerDITTOLAE, self).compute_loss(model, inputs, return_outputs=True) 
            ...
            # for name, param in model_params.items():
                ...
                if 'ia3_l_2_2' in name:
                    target_name = name.replace('ia3_l_2_2', 'ia3_l_1_2')
                ...
                if (
                    ...
                    with torch.no_grad():
                        ...
                        for name, param in model_params.items():
                            if not param.requires_grad:
                                continue
                            if 'ia3_l_1_2' in name:
                                target_name = name.replace('ia3_l_1_2', 'ia3_l_1_1')
                                model_params[target_name].copy_(self.ema_ratio*model_params[target_name] + (1-self.ema_ratio)*param)
                        ...
                        with torch.no_grad():
                            ...
                            for name, param in model_params.items():
                                if not param.requires_grad:
                                    continue
                                if 'ia3_l_2_2' in name:
                                    target_name = name.replace('ia3_l_2_2', 'ia3_l_2_1')
                                    model_params[target_name].copy_(self.ema_ratio*model_params[target_name] + (1-self.ema_ratio)*param)
