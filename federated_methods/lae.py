...
def LAE_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict):
    task_id = extra_state_dict_dict['task_id'] if 'task_id' in extra_state_dict_dict else None
    ema_ratio = training_args.ema_ratio
    trainer = LLaVATrainerLAE(model=model,
        tokenizer=tokenizer,
        args=training_args,
        packing=True,
...
class LLaVATrainerLAE(LLaVATrainerFEDAVG):
    def __init__(self, ema_ratio=0.996, **kwargs):
        super(LLaVATrainerLAE, self).__init__(**kwargs)
    ...
    def _inner_training_loop(
        ...
        # OURS:
        self.model.set_lae_state('lora2')
