...
def apfl_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict):
    trainer = LLaVATrainerAPFL(model=model,
        tokenizer=tokenizer,
        args=training_args,
        packing=True,
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # apfl: update alpha
                    grad_alpha = 0
                    model_param = OrderedDict(self.model.named_parameters())
                    for name, param in self.model.named_parameters():
                        # only trainable parameters
                        if not param.requires_grad or 'local_mm_projector' not in name or 'lora2' not in name:
                            continue
